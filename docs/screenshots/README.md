# Screenshots

Pasta de destino para os prints referenciados no `README.md` / `README.en.md` principal do projeto.

Nomes de arquivo esperados (PNG, mesma pasta):

| Arquivo | Tela |
| --- | --- |
| `login.png` | `TelaLogin` |
| `cadastro-inicial.png` | `TelaCadastroInicial` |
| `home.png` | `TelaHome` (agenda pendente) |
| `minha-barbearia.png` | `TelaHome` → aba "Minha Barbearia" |
| `historico.png` | `TelaHome` → aba "Histórico" |
| `novo-agendamento.png` | `TelaNovoAgendamento` |

Basta salvar os arquivos com esses nomes nesta pasta; os links no README já apontam para eles.
